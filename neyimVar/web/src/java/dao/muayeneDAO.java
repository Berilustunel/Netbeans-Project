...
public class muayeneDAO extends dbConnection {
    ...
    public muayene getById(int id){
        ...
        try{
            ...
            rs.next();
            m = new muayene(rs.getInt("muayene_id"), rs.getDate("muayene_tarih"), rs.getTime("muayene_saat"), rs.getString("ad"), rs.getString("soyad"));
        }catch (Exception e){
    ...
    public void create(muayene h) {
        try {
            ...
            st.execute("insert into muayene (muayene_id, muayene_tarih, muayene_saat) "
                    + "values('" + h.getMuayene_id()+ "','" + h.getMuayene_tarih()+ "', '" + h.getMuayene_saat()+ "', '" + h.getAd()+ "', '" + h.getSoyad()+ "')");
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
    }
    ...
    public List<muayene> read() {
        ...
        try {
            ...
            while (rs.next()) {
                muayene tmp = new muayene(rs.getInt("muayene_id"), rs.getDate("muayene_tarih"), rs.getTime("muayene_saat"), rs.getString("ad"), rs.getString("soyad"));
                list.add(tmp);
            }
